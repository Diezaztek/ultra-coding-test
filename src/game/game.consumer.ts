...
export class GameConsumer {
  ...
  substractMonths(monthsToSubstract: number) {
    ...
    const dateDeleteGames = new Date(
      today.setMonth(today.getMonth() - monthsToSubstract),
    );
  ...
  async readOperationJob(): Promise<void> {
    this.logger.log(
      'Game catalog adjusment by date old started',
      'GameConsumer',
    );
    ...
    try {
      ...
      this.logger.verbose(
        `Games successfully deleted. ${gamesDeleted.length} games affected`,
        'GameConsumer',
      );
      ...
      this.logger.verbose(
        `Games successfully updated its prices. ${gamesUpdated.affected} games affected`,
        'GameConsumer',
      );
    } catch (error) {
